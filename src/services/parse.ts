...
function DnsServerFromClash(value: unknown, prefix: string, index: number, label = 'DNS'): AnyObject | null {
    if (typeof value !== 'string' || !value.trim()) return null;

    const tag = `${prefix} / ${label} ${index + 1}`;
...
function ExtractClashDns(payload: unknown, nodes: Node[], profileName: string): ProfileDns | undefined {
    ...
    const defaultNameservers = payload.dns['default-nameserver'];
    ...
    const bootstrapServers = Array.isArray(defaultNameservers)
        ? defaultNameservers
            .map((server, index) => DnsServerFromClash(server, profileName, index, 'Bootstrap DNS'))
            .filter((server): server is AnyObject => server !== null)
        : [];

    const nodeDomains = ExtractNodeDomains(nodes);
    console.info(`[parse] ${profileName} clash dns candidates nameservers=${nameservers.length} servers=${servers.length} bootstrapServers=${bootstrapServers.length} nodeDomains=${nodeDomains.length}`);
    ...
    return { servers, bootstrapServers, nodeDomains };
}
...
export async function ParseProfile(name: string, content: string, format: UpstreamFormat, encoding: UpstreamEncoding | undefined, type: UpstreamType): Promise<Profile> {
    ...
    console.info(`[parse] profile ${name} type=${type} format=${format} nodes=${nodes.length} dns=${dns ? `yes servers=${dns.servers.length} bootstrap=${dns.bootstrapServers?.length ?? 0} domains=${dns.nodeDomains.length}` : 'no'}`);
    return profile;
}
